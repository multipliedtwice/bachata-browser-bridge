...
CSS, XPath, JavaScript, URLs, coordinates, arbitrary text, and unknown IDs are rejected. JSON receives at most one repair attempt and must then pass the closed schema check in `src/content/generic/schemaGuard.ts`. That check is interpreted, not compiled: content scripts run under the page's Content Security Policy, which blocks generated code. The healer queue permits one active request with a bounded waiting queue.
...
The VS Code extension decides which model heals and turns healing on. Healing is off by default. Turn it on in the Agents view under **Local models → Selector healing**, or with `bachata.browserSelectorHealingEnabled`. When healing is turned on, and again at each start, the extension sends the chosen model a `bachata-dom-heal-v1` probe page: a chat textarea, a conversation region, a send button, a reply, and three decoys (a search box, a link, a settings button). Only a model that picks the chat controls and leaves the decoys alone is handed to the Bridge, as soon as its check passes. The action interpreter's check does not count for healing.
